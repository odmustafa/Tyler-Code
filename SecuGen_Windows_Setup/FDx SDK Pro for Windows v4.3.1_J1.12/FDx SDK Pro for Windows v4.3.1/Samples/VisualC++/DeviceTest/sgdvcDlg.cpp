...
void CSgdvcDlg::OnEnumBtn() 
{
	...
	if (m_Sensor)
	{
		...
		for (int i = 0; i < (int)dev_nums; i++)
		{
			...
			else if (m_EnumData[i].DevName == SG_DEV_FDU06AP)
				dev_name = "USB FDU06AP (UPx-AP)";
			else if (m_EnumData[i].DevName == SG_DEV_FDU08AL)
				dev_name = "USB FDU08AL (U20AL)";
...
void CSgdvcDlg::Initialize(bool is_usb)
{
	...
	if ((m_Sensor))
	{
		...
		if (is_usb)
		{
			...
			//error = m_Sensor->Init(SG_DEV_AUTO);
...
void CSgdvcDlg::InitUI(bool is_usb) 
{
	...
	{
		if (
			(m_EnumData[m_CurDev].DevName == SG_DEV_FDU08AL) ||
			(m_EnumData[m_CurDev].DevName == SG_DEV_FDU06AP) ||
