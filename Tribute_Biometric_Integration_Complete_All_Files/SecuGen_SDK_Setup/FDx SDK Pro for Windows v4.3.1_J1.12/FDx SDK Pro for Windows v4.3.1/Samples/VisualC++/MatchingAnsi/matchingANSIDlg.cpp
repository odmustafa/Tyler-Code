...
BOOL CMatchminDlg::OnInitDialog()
{
   ...
   m_CBDevName.AddString("USB FDU08AL(U20AL)");
   m_CBDevName.AddString("USB FDU06AP(UPx-AP)");
   ...
   m_CBDevName.SetCurSel(0); // USB U20AP
...
void CMatchminDlg::OnInitbutton() 
{
   ...
   if (devstr == "USB FDU08AL(U20AL)")		// USB: FDU08AL(U20AL)
	   devname = SG_DEV_FDU08AL;
   else if (devstr == "USB FDU06AP(UPx-AP)")// USB: FDU06AP(UPx-AP)
	   devname = SG_DEV_FDU06AP;
   ...
   {
      m_ResultEdit.Format("Library initialization failed. Error = %d", err);
      UpdateData(false);   
      return;
   }
...
void CMatchminDlg::OnRegbutton() 
{
   ...
   if ((err == SGFDX_ERROR_NONE) && matched)
   {
      ...
      // Display ANSI template information         
      int i;
      for (i = 0; i < 11; i++)
         m_BtnEnrolledFinger[i]->SetCheck(false);
...
void CMatchminDlg::OnMatbutton() 
{
   ...
   {
	   m_ResultEdit.Format("MatchAnsiTemplate() failed. Error = %d ", err);
   }
